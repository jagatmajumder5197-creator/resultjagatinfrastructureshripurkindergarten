...
function loadClassDropdown() {
  ...
  classes.sort((a, b) => String(a).localeCompare(String(b)));
...
document.getElementById('classSelect').addEventListener('change', function () {
  ...
  students.sort((a, b) => String(a.STUDENTS_NAME || '').localeCompare(String(b.STUDENTS_NAME || '')));
  ...
  const classStudents = allStudents
    ...
    .map(s => {
      ...
      subjects.forEach(sub => {
        const fm = Number(s[sub.fm] || 0);
        if (fm > 0) {
          total += Number(s[sub.written] || 0) + Number(s[sub.oral] || 0);
        }
      });
